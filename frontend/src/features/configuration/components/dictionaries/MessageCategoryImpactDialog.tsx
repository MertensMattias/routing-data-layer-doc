...
import type { MessageCategoryImpact } from '@/api/types';
import { getMessageCategoryImpact } from '@/services/configuration';
import { getApiErrorMessage } from '@/api/client';

interface MessageCategoryImpactDialogProps {
  open: boolean;
...
export function MessageCategoryImpactDialog({
  open,
  onOpenChange,
  categoryCode,
  onDelete,
}: MessageCategoryImpactDialogProps) {
  const [impact, setImpact] = useState<MessageCategoryImpact | null>(null);
  ...
  const loadImpact = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getMessageCategoryImpact(categoryCode);
      setImpact(data);
    } catch (err: unknown) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Review what would be affected if this category is deactivated.
          </DialogDescription>
        </DialogHeader>
              ...
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <p className="text-sm mt-1">
                    This category is not being used and can be safely deactivated.
                  </p>
                </AlertDescription>
              </Alert>
            )}
